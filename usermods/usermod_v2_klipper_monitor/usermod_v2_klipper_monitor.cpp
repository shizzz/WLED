...
{
    _queueManager = new QueueManager(checkInterval); // 200ms interval
    for (int i = 0; i < PRESET_COUNT; i++)
    {
        if (_presetSettings[i].active || !_automaticSwitch)
        {
            _queueManager->addPreset(&_presetSettings[i]);
        }
    }
    _initDone = true;
}
...
{
    if (!_enabled || _state != IDLE || !_queueManager->getActive())
    {
        return;
    }
    
    if (_queueManager->activePreset()) {
        update(); 
    }
}
...
void KlipperMonitor::handleOverlayDraw()
{
    ...
    auto painter = createPainter(_queueManager->activePreset()->effect);
    painter->paint(_queueManager->activePreset(), _parseResult);
}

void KlipperMonitor::addToConfig(JsonObject &root) {
    JsonObject top = root.createNestedObject(FPSTR(_name));
    top[F("enabled")] = _enabled;
    top[F("auto switch")] = _automaticSwitch;
    top[F("host")] = _host;
    top[F("port")] = _port;
    ...
    for (int i = 0; i < PRESET_COUNT; i++)
    {
        ...
        modeJson[F("active")] = _presetSettings[i].active;
...
bool KlipperMonitor::readFromConfig(JsonObject &root) {
    ...
    configComplete &= getJsonValue(top[F("auto switch")], _automaticSwitch, true);
    ...
    for (int i = 0; i < PRESET_COUNT; i++)
    {
        ...
        configComplete &= getJsonValue(jsonSetting[F("active")], _presetSettings[i].active, true);
        ...
        _presetSettings[i].url = "/printer/objects/query?";
        _presetSettings[i].url += _presetSettings[i].entity;
    }
...
void KlipperMonitor::parseResponse(String response)
{
    ...
    if (bodyPos > 0) {
        ...
        if (jsonStr[0] == '{' || jsonStr[0] == '[') {
            ...
            auto parser = createParser(_queueManager->activePreset()->type);
            parser->parse(jsonStr.c_str(), _queueManager->activePreset());
        } else {
